...
function track(j: number, sx: number): PartSpec[] {
  const x = sx * 0.2;
  const parts: PartSpec[] = [{ j, shape: 'box', size: [0.14, 0.66, 0.62], at: [x, -0.24, 0], color: TRACK }];
  for (let i = 0; i < 7; i++)
